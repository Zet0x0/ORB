...
set(TABLER_ICONS_ARCHIVE_PREFIX "tabler-icons-${TABLER_ICONS_VERSION}")
...
if(NOT _dest_version STREQUAL TABLER_ICONS_VERSION)
    message(
        STATUS
            "Extracting tabler-icons ${TABLER_ICONS_VERSION} outline SVGs to ${TABLER_ICONS_DEST_DIR}..."
    )
    ...
    file(
        ARCHIVE_EXTRACT
        INPUT
        "${TABLER_ICONS_ARCHIVE_PATH}"
        DESTINATION
        "${TABLER_ICONS_DEST_DIR}"
        PATTERNS
        "${TABLER_ICONS_ARCHIVE_PREFIX}/icons/outline/*")

    set(_extracted_icons_dir
        "${TABLER_ICONS_DEST_DIR}/${TABLER_ICONS_ARCHIVE_PREFIX}/icons")

    if(NOT EXISTS "${_extracted_icons_dir}/outline")
        message(
            FATAL_ERROR
                "tabler-icons extraction produced no outline SVGs (looked for ${_extracted_icons_dir}/outline)"
        )
    endif()

    file(RENAME "${_extracted_icons_dir}" "${TABLER_ICONS_DEST_DIR}/icons")
    file(REMOVE_RECURSE
         "${TABLER_ICONS_DEST_DIR}/${TABLER_ICONS_ARCHIVE_PREFIX}")

    file(WRITE "${_dest_version_file}" "${TABLER_ICONS_VERSION}")
else()
    message(
        STATUS
            "Using already-extracted tabler-icons at ${TABLER_ICONS_DEST_DIR}")
endif()
